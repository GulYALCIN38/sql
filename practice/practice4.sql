...
--soru:meslekler tablosunda ilk 2 veriden sonra 5 veriyi getirin
select * from meslekler offset 2 limit 5;--"offset" atlama yapmak icin kullanilir yani ilk 2 satiri atla demek
...
from filmler f left join aktorler a
...
select film_name, category, actor_name
from aktorler a right join fil
mler f
on f.film_id = a.film_id

-- SORU2: Tüm actor_name'leri ve film_name'lerini listeleyiniz.
select film_name,  actor_name from 
aktorler a left join filmler f
on f.film_id = a.film_id
...
select * from 
qa_dersler q left join developer_dersler d
on q.ders_id=d.ders_id



select * from 
qa_dersler q right join developer_dersler d
on q.ders_id=d.ders_id
